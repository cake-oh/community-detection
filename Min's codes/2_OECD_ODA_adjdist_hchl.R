...
setwd("E:/2_my_project/3_metacoupling/1_country_list/final")

dist <- read.csv("World_countries_eez_matrix_ISO3__1980_2010s.csv", header=T, stringsAsFactors=FALSE)
country <- read.csv("UNCTAD_country_code_hchl_dac.csv", header=T, stringsAsFactors=FALSE)
country.iso3 <- country$iso3

country.sub <- subset(country, !is.na(dac.code))


setwd("E:/2_my_project/3_metacoupling/2_data_process/OECD_ODA/")
input <- read.csv("OECD_ODA_select_new_cnst21.csv", header=T, stringsAsFactors=FALSE)
...
# national
write.csv(input.hchl3.dist.sub, paste0("ODA_Flows_national.csv"), row.names=F)
...
# income
write.csv(input.hchl3.dist.global.hl.flow, paste0("ODA_Flows_global_HL.csv"), row.names=F)

write.csv(input.hchl3.dist.global.hl.exp, paste0("ODA_Flows_global_HL_exp.csv"), row.names=F)

write.csv(input.hchl3.dist.global.hl.imp, paste0("ODA_Flows_global_HL_imp.csv"), row.names=F)

# hotspot
write.csv(input.hchl3.dist.global.hc, paste0("ODA_Flows_global_HC.csv"), row.names=F)

write.csv(input.hchl3.dist.global.hc.exp, paste0("ODA_Flows_global_HC_exp.csv"), row.names=F)

write.csv(input.hchl3.dist.global.hc.imp, paste0("ODA_Flows_global_HC_imp.csv"), row.names=F)


# income & hotspot
write.csv(input.hchl3.dist.global.hchl, paste0("ODA_Flows_global_HCHL.csv"), row.names=F)

write.csv(input.hchl3.dist.global.hchl.exp, paste0("ODA_Flows_global_HCHL_exp.csv"), row.names=F)

write.csv(input.hchl3.dist.global.hchl.imp, paste0("ODA_Flows_global_HCHL_imp.csv"), row.names=F)
